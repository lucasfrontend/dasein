...
const store = new Vuex.Store({
  ...
  getters: {
    ...
    userRoles: (state) => state.userLoged.role,
  ...
  mutations: {
    setDataTable(state, newData) {
      console.log('Mutación setDataTable llamada con:', newData);
      state.dataTable = newData;
      state.dataTableKey = Date.now();
      //Vue.set(state, 'dataTable', newData);
      //state.dataTable = [...newData];
    },
  ...
  actions: {
    async getDataTable({ commit }) {
      ...
      try {
        const response = await axios.get(`${process.env.VUE_APP_TANDAS}?today=true`);
