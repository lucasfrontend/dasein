...
import { isIos, isSafari, isMac, isClient,isChrome, isMobileDetect, isPwa, goToFullscreen, coolFormateDate, isMozilla, calcDistance } from "./helpers/index";
...
import BackToTop from "./views/app/BackToTop.vue";
...
export default {
  ...
  mounted() {
    ...
    */
    ...
    // test PRUEBA NUEVO
    eventBus.$on("getTandas", function () {
      console.log("getTandas en APPP")
      vm.getTandas();
    });
  ...
  methods: {
    dispositivos(){
      const locationChascomus = {
        lat: -35.5803, // Latitud de Chascomús
        lng: -58.0198, // Longitud de Chascomús
      };
      let resultDistan = calcDistance(locationChascomus)
      console.log("resultDistan", resultDistan)
...
userDB {
  ...
  _doc: {
    ...
    last_name: 'martinez',
    alias: 'barri',
    email: 'barripackers@sdch.com',
    password: '$2b$10$BYdpVCu7nL1OkPF7NxJv3.NbXUfDIdIs6WaraBsAf2Jsy24SA4U9a',
    av_c: { av_total: 0, _id: new ObjectId("653885aecf9bc52f3876f79d") },
    role: [],
    balance: 0,
    beer_bal: 0,
    _id: new ObjectId("653885aecf9bc52f3876f79e"),
    license_numbers: [],
    transactions: [],
    b_transactions: [],
    feePaid: [],
