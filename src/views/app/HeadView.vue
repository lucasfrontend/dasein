<template>
  <v-app-bar app color="gray" dark hide-toolbar>
    <v-row class="align-center">
      ...
      <v-col cols="6" class="d-flex align-center justify-center">
        <div class="align-self-center" style="color: #ffff00;">
          <div v-if="$route.name !== 'dashboard'" class="text-center  ml-1" style="font-size: 0.8rem;"> {{ title }}</div>
          <div style="font-size: 0.8rem;" v-if="$route.name == 'dashboard' && $store.state.userLoged">¡Hola {{ $store.state.userLoged.first_name }}!</div>
        </div>
      </v-col>
