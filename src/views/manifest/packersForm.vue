...
export default {
  ...
  mounted() {
    ...
    if (id) {
      this.id = id
      this.$http.get(`${process.env.VUE_APP_PACKERS}/${id}`)
        .then(response => {
  ...
  methods: {
    savePacker() {
      this.loading = true;
      const url = this.id ? `${process.env.VUE_APP_PACKERS}/${this.id}` : process.env.VUE_APP_PACKERS;
    ...
    deletePacker(){
      if (this.id) {
        this.loading = true;
        this.$http.delete(`${process.env.VUE_APP_PACKERS}/${this.id}`)
          .then(response => {
